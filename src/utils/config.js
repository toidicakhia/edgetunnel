...
export function getTransportPathParamValue(config = {}, nodePath = '/', asOptimalSubGenerator = false) {
...
export async function singboxSubscriptionHotPatch(SingBox_originalsubscriptionContent, config_JSON = {}) {
	const uuid = config_JSON?.UUID || null;
...
export async function logRequest(env, request, accessIP, requestType = "Get_SUB", config_JSON, writeKVLog = true) {
	try {
		const currentTime = new Date();
		const logEntry = { TYPE: requestType, IP: accessIP, ASN: `AS${request.cf.asn || '0'} ${request.cf.asOrganization || 'Unknown'}`, CC: `${request.cf.country || 'N/A'} ${request.cf.city || 'N/A'}`, URL: request.url, UA: request.headers.get('User-Agent') || 'Unknown', TIME: currentTime.getTime() };
...
export async function readConfigJSON(env, hostname, userID, UA = "Mozilla/5.0", resetConfig = false) {
