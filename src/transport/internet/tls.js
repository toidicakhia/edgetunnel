...
export const shouldIgnoreTlsAlert = (fragment) =>
...
export const readUint24 = (buffer, offset) =>
...
export async function hmac(hash, key, data) {
	const cryptoKey = await crypto.subtle.importKey('raw', key, { name: 'HMAC', hash }, !1, [
		'sign',
	]);
...
export async function hkdfExpandLabel(hash, secret, label, context, length) {
	...
	return (async function (hash, secret, info, length) {
		...
		hash,
		secret,
		tlsBytes(uint16be(length), fullLabel.length, fullLabel, context.length, context),
		length
	);
}
...
export function poly1305Mac(key, message) {
	...
	tagValue =
		(tagValue +
			sKey.reduce((total, byte, index) => total + (BigInt(byte) << BigInt(8 * index)), 0n)) &
		((1n << 128n) - 1n);
...
export function chacha20Poly1305Decrypt(key, nonce, ciphertext, additionalData) {
	...
	const tag = ciphertext.slice(-16),
		...
		macData = new Uint8Array(
			additionalData.length + aadPadding + encryptedData.length + ciphertextPadding + 16
		);
...
export function parseEncryptedExtensions(body) {
	...
	for (; offset + 4 <= extensionsEnd;) {
		...
		if (
			((offset += 2),
			extensionType === EXT_APPLICATION_LAYER_PROTOCOL_NEGOTIATION && extensionLength >= 3)
		) {
...
export function buildClientHello(
	...
	(enableTls13 && cipherIds.push(4865, 4866, ...(chacha ? [4867] : [])),
		enableTls12 &&
			cipherIds.push(49199, 49200, 49195, 49196, ...(chacha ? [52392, 52393] : [])));
...
export class TlsClient {
	...
	async encryptTls13Handshake(plaintext) {
		...
		if (!this.clientHandshakeCryptoKey)
			this.clientHandshakeCryptoKey = await importAesGcmKey(this.clientHandshakeKey, [
				'encrypt',
			]);
			...
			plaintext = this.cipherConfig.chacha
				...
				: await aesGcmDecryptWithKey(
						this.serverAppCryptoKey ||
							(this.serverAppCryptoKey = await importAesGcmKey(this.serverAppKey, [
								'decrypt',
							])),
						nonce,
						ciphertext,
