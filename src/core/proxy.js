...
export async function socks5Connect(targetHost, targetPort, initialData, tcpConnector, parsedSocks5) {
...
export async function httpConnect(targetHost, targetPort, initialData, HTTPSproxy = false, tcpConnector, parsedSocks5) {
	const { username, password, hostname, port } = parsedSocks5 || {};
...
export async function httpsConnect(targetHost, targetPort, initialData, tcpConnector, parsedSocks5) {
	const { username, password, hostname, port } = parsedSocks5 || {};
	const encoder = new TextEncoder();
	const decoder = new TextDecoder();
	let tlsSocket = null;
	...
	try {
		try {
			...
			if (!/cipher|handshake|TLSAlert|ServerHello|Finished|Unsupported|MissingTLS/i.test(error?.message || `${error || ''}`)) throw error;
...
export const proxyProtocolDefaultPorts = { socks5: 1080, http: 80, https: 443, turn: 3478, sstp: 443 };
...
export const SOCKS5accountBase64regex = /^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$/i, IPv6bracketRegex = /^\[.*\]$/;
