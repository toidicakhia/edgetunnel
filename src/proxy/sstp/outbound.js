/**
 * src/proxy/sstp/outbound.js
 * SSTP chain outbound handler.
 */

import { Handler } from '../../features/outbound.js';
import { attachSocket, createRequestTCPConnector } from '../../transport/internet/tcp.js';
import { sstpConnect } from './client.js';

export class SstpChainHandler extends Handler {
	/**
	 * @param {string} tag
	...
	async dial(ctx, dest) {
		...
		if (!connector) throw new Error('sstp: no connector');
		const proxy = this.proxyParams;
		if (!proxy?.hostname) throw new Error('sstp: no proxy configured');
		return sstpConnect(proxy, dest.address, dest.port, connector);
	}

	async dispatch(ctx, link) {
		const dest = ctx.outbound?.target;
		if (!dest) throw new Error('sstp: no target');
