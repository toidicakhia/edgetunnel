...
import { concatByteData, log, parseToArray, randomPath, replaceWildcardWithRandomChars } from './utils/helpers.js';
import {
...
export default {
	async fetch(request, env, ctx) {
		...
		setProxyConcurrentDialCount(Math.max(1, Number(env.PROXY_CONCURRENT_DIAL) || PROXY_CONCURRENT_DIAL_COUNT));
		setTCPConcurrentDialCount(Math.max(1, Number(env.TCP_CONCURRENT_DIAL) || TCP_CONCURRENT_DIAL_COUNT));
		if (!env.TCP_CONCURRENT_DIAL && TCP_CONCURRENT_DIAL_COUNT !== 1 && identifyISP(request) === 'cmcc')
		...
		if (cachedSocks5Whitelist === null) {
			if (env.GO2SOCKS5) setSocks5Whitelist([...new Set(socks5Whitelist.concat(await parseToArray(env.GO2SOCKS5)))]);
			setCachedSocks5Whitelist(socks5Whitelist);
		} else setSocks5Whitelist(cachedSocks5Whitelist);
		if (accessPath === 'version') {// versionAPI
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (caseSensitiveAccessPath === encryptionSecret && encryptionSecret !== 'Do not modify this default secret key') {//quickSubscription
				...
				} else if (accessPath === 'admin' || accessPath.startsWith('admin/')) {//validatecookiethenRespondAdminPage
					const cookies = request.headers.get('Cookie') || '';
					const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
					...
					if (accessPath === 'admin/log.json') {// logContent
					...
					} else if (caseSensitiveAccessPath === 'admin/getCloudflareUsage') {// queryUsage
						try {
						...
						if (url.searchParams.get('url')) {
							...
							try {
								...
								const fetchOptimalAPIResult = await fetchOptimalAPI([pendingVerifyOptimalURL], url.searchParams.get('port') || '443');
					...
					} else if (request.method === 'POST') {// processKVOperation（POSTRequest）
					...
					} else if (accessPath === 'admin/config.json') {// processAdmin/config.jsonRequest，returnJSON
						return new Response(JSON.stringify(config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json' } });
					} else if (caseSensitiveAccessPath === 'admin/ADD.txt') {// processAdmin/ADD.txtRequest，returnlocalOptimalIP
				...
				} else if (accessPath === 'logout' || uuidRegex.test(accessPath)) {//clearcookieandRedirectToLoginPage
					...
					if (userClientRequestingSub || subConverterRequestingSub || asOptimalSubGenerator) {
						...
						if (config_JSON.CF.Usage.success) {
							...
							responseHeaders["Subscription-Userinfo"] = `upload=${pagesSum}; download=${workersSum}; total=${total}; expire=4102329600`; // 2099-12-31ExpiryTime
						}
						...
						if (subscriptionType === 'mixed') {
							...
							if (!url.searchParams.has('sub') && config_JSON.optSubGenerator.local) { // localSubscriptionGeneration
								...
								otherNodesLINK = mergedOtherNodeArray.length > 0 ? mergedOtherNodeArray.join('\n') + '\n' : '';
							...
							} else { // optimalSubscriptionGenerator
